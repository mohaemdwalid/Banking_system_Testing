...
public class CreateClientController implements Initializable{
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        create_client_btn.setOnAction(actionEvent -> createClientAccounts());
    ...
    private void createClientAccounts(){
    ...
    private void createAccount(String accountType){
        double balance_ch = Double.parseDouble(ch_amount_fld.getText());
        double balance_sv = Double.parseDouble(sv_amount_fld.getText());
        String firstSection = "3201";
        String lastSection = Integer.toString((new Random()).nextInt(9999)+ 1000);
        String accountNumber = firstSection + " " + lastSection;
        if (accountType.equals("Checking")){
            Model.getInstance().getDataBaseDriver().createCheckingAccount(payeeAddress,accountNumber,10,balance_ch);
        }else {
            Model.getInstance().getDataBaseDriver().createSavingAccount(payeeAddress,accountNumber,2000,balance_sv);
        }
    }
